...
/**
 * Track#setAnonymousMode
 *
 * Sets the user's signed mode
 */
exports.setSignedMode = function setSignedMode (signedMode) {
  if (signedMode) {
    cookie.setItem('__signed', 'true')
  } else {
    cookie.setItem('__signed', 'false')
  }
  return this
}

/**
 * Track#getAnonymousMode
 *
 * Tells whether or not the user is anonymous
 */
exports.getSignedMode = function getSignedMode () {
  return cookie.getItem('__signed') === 'true'
}
...
exports.getTrackValues = function getTrackValues () {
  var result = {}
  var isAnonymous = !this.getSignedMode()
  _.forIn(this.client.track.values, function (value, key) {
    if (
      value &&
      !(isAnonymous && (value === 'td_ip' || value === 'td_client_id'))
    ) {
