 ...
 * config.storage.disableLocalStorage (Boolean)
 *    - disables storage of the cookie values in local storage if it's required
 * config.track (Object)
...
exports.configure = function configure (config) {
  ...
  if (_.isNumber(config.clientId)) {
    ...
    if (config.storage && config.storage.name) {
      config.clientId = setCookie.getCookie(config.storage, config.storage.name)
    }
...
exports.resetUUID = function resetUUID (suggestedStorage, suggestedClientId) {
  ...
  if (storage) {
    if (storage.expires) {
      setCookie.setCookie(storage, storage.name, undefined)
      setCookie.setCookie(storage, storage.name, this.client.track.uuid)
    }
  }
