...
/**
 * @param {object} config - Configuration object
 * @param {string} config.cdpHost - The host to use for the Personalization API, defaults to "profile-api.treasuredata.com"
 * @param {string} config.token - Audience token to use
 * @param {string} config.index - Index key, for example "td_client_id"
 * @param {function} [success] - Callback for receiving the user key and segments
 * @param {function} [error] - Callback for when sending the event is unsuccessful
 *
 * @example
 ...
 * }, successCallback, errorCallback)
*/
function lookupProfile (config, successCallback, errorCallback) {
  config = config || {}

  var cdpHost = config.cdpHost || 'profile-api.treasuredata.com'

  invariant(
    config.token && typeof config.token === 'string',
    'token must be a string, received "' + String(config.token) + '"'
  )

  invariant(
    config.index && typeof config.index === 'string',
    'index must be a string, received "' + String(config.index) + '"'
  )

  var url = 'https://' + cdpHost + '/v1/profile/lookup?token=' + config.token + '&index=' + config.index
  api
    .get(url, {
      ignoreDefaultHeaders: true,
      headers: {
