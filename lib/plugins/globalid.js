/**
 * Treasure Global ID
 */

// Modules
var jsonp = require('jsonp')
var invariant = require('../utils/misc').invariant
var noop = require('../utils/misc').noop
var global = require('global')
var cookie = require('../vendor/js-cookies')

function cacheSuccess (result, cookieName, storeConsentByLocalStorage) {
  if (storeConsentByLocalStorage) {
    global.localStorage[cookieName] = result['global_id']
  } else {
    cookie.setItem(cookieName, result['global_id'], 6000)
  }
  return result['global_id']
}

function configure () {}

function fetchGlobalID (success, error, forceFetch) {
  success = success || noop
  error = error || noop
  var storeConsentByLocalStorage = this.client.storeConsentByLocalStorage
  var cookieName = this.client.globalIdCookie
  var cachedGlobalId
  if (storeConsentByLocalStorage) {
    cachedGlobalId = global.localStorage[this.client.globalIdCookie]
  } else {
    cachedGlobalId = cookie.getItem(this.client.globalIdCookie)
  }
  if (cachedGlobalId && !forceFetch) {
    return setTimeout(function () {
      success(cachedGlobalId)
    }, 0)
  }

  var url = 'https://' + this.client.host + '/js/v3/global_id'

  invariant(
    this.client.requestType === 'jsonp',
    'Request type ' + this.client.requestType + ' not supported'
  )

  jsonp(
    url,
    {
      prefix: 'TreasureJSONPCallback',
      timeout: this.client.jsonpTimeout
    },
    function (err, res) {
      return err ? error(err) : success(cacheSuccess(res, cookieName, storeConsentByLocalStorage))
    }
  )
}

module.exports = {
  cacheSuccess: cacheSuccess,
  configure: configure,
  fetchGlobalID: fetchGlobalID
}
